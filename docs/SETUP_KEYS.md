...
| `NEXT_PUBLIC_ADSENSE_CLIENT` | 애드센스 광고 | ⬜ 선택 | ⬜ 선택 | 선택 → 4번 |
| **Google Search Console** | 검색 노출/색인 | — | — | **등록 필요 → 2번 (수익보다 먼저!)** |

➡️ **지금 해야 할 세 가지**: ① 관리자 로그인(세션 시크릿 없음 → 1번) ② **구글 검색 노출(사이트맵 미제출 → 2번, 가장 중요)** ③ 알리 제휴 수익(앱키 없음 → 3번).

> 📌 사이트 코드/콘텐츠 측면의 SEO는 **이미 전부 완료**됐습니다(아래 "이미 끝난 것" 참고). 남은 건 **구글에 "우리 사이트 봐줘"라고 등록**하는 일뿐이고, 이건 코드가 아니라 구글 계정으로 직접 하셔야 합니다 → **2번**.
>
> **이미 끝난 것 (코드 측 SEO, 손댈 것 없음):**
> - `sitemap.xml` 자동 생성 — 현재 **126개 URL**(글 125 + 홈) 라이브, 글 추가 시 자동 반영
> - `robots.txt` — 크롤러 허용 + 사이트맵 위치 명시
> - 페이지별 `keywords` 메타 — "Roborock S7 Error 9", "S7 수리/고장" 등 영문+한글 키워드
> - 구조화 데이터(JSON-LD) — `HowTo`(단계별 수리) + `FAQPage`(자주 묻는 질문) → 구글 리치결과(별점/단계 카드) 노출용
...
## 2. Google Search Console — 키워드 "실제 노출" (가장 중요, 외부발급 불필요)

사이트맵·robots·키워드·구조화데이터는 **코드에 이미 다 들어가 있습니다**. 하지만 구글은 "등록되지 않은 새 사이트"를 바로 크롤링하지 않습니다. **검색에 실제로 뜨려면 Search Console에 사이트를 등록하고 사이트맵을 제출**해야 합니다. 무료이고 키 발급도 필요 없습니다. 도메인 소유 확인만 하면 됩니다.

### 2-1. Search Console 접속 & 속성 추가
1. <https://search.google.com/search-console> 접속 (구글 계정 로그인 — koreadobby@gmail.com).
2. 좌상단 **속성 추가** → **도메인** 방식 선택 → `fixfind.cc` 입력.
   - (간단히 하려면 **URL 접두어** 방식에 `https://fixfind.cc` 입력해도 됨. 도메인 방식이 모든 하위 경로를 한 번에 커버해서 권장.)

### 2-2. 소유권 확인 (DNS TXT — 도메인 등록업체에서)
1. Search Console가 **TXT 레코드 값**(`google-site-verification=...`)을 보여줍니다.
2. 도메인을 산 곳(가비아/Cloudflare/Namecheap 등) DNS 관리에서 **TXT 레코드 추가**:
   - 호스트/이름: `@` (또는 공란) / 타입: `TXT` / 값: 복사한 `google-site-verification=...`
3. 저장 후 Search Console로 돌아와 **확인** 클릭 (DNS 전파에 몇 분~수십 분).
   - ⚠️ DNS 변경은 제가 대신 못 합니다(네트워크/도메인 설정은 직접). 화면 그대로 따라 하시면 됩니다.

### 2-3. 사이트맵 제출
1. 확인 완료 후 좌측 메뉴 **Sitemaps(사이트맵)**.
2. "새 사이트맵 추가" 칸에 `sitemap.xml` 입력 → **제출**.
   - 전체 주소: `https://fixfind.cc/sitemap.xml` (현재 126개 URL 라이브 확인됨).
3. 상태가 **성공**으로 바뀌면 끝. 구글이 125개 글을 차례로 색인합니다(며칠~2주 소요).

### 2-4. (선택) 색인 빠르게 — URL 검사
- 중요한 글 몇 개는 상단 **URL 검사** 칸에 주소를 넣고 **색인 생성 요청**을 누르면 우선 처리됩니다.
- 예: `https://fixfind.cc/roborock/s7-error-9`

### 2-5. 검증 (등록 후 며칠 뒤)
- Search Console → **페이지** 리포트에서 "색인 생성됨" 개수 증가 확인.
- 구글에서 `site:fixfind.cc` 검색 → 등록된 글 개수 확인.
- `Roborock S7 Error 9 fix` 등으로 검색 시 노출되기 시작(경쟁/시간에 따라 수일~수주).

> 💡 콘텐츠 측 SEO는 손댈 게 없습니다. **이 2번(검색 등록)이 "키워드 실제 노출"의 진짜 스위치**입니다.

---

## 3. AliExpress 제휴 API — 수익 링크 활성화 (외부 계정 발급 필요)

지금은 제휴키가 없어 상품 페이지에 **"알리에서 부품 검색" 폴백 링크**만 노출됩니다(작동은 함). 아래를 완료하면 **실제 커미션이 붙는 최저가 상품 링크**로 자동 교체됩니다.

### 3-1. 계정 만들기 (직접 — 제가 대신 못 만듭니다)
...
### 3-2. 개발자 앱 생성 → App Key / App Secret
...
- 신규 글은 영문+한글 가이드가 함께 생성되고, **사이트맵에 자동 등록**됩니다(`sitemap.xml` 자동 갱신).
- ⚠️ 단, 구글에 실제로 노출되려면 **2번(Search Console 등록)이 선행**돼야 합니다. 등록만 해두면 이후 추가되는 글은 사이트맵을 통해 자동으로 색인됩니다.
